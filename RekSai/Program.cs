    ...
    class Program
    {
        ...
        private static void Combo()
        {
            ...
            if (burrowed)
            {
                if (Q2.IsReady() && useQ2)
                {
                    foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsEnemy && !x.IsZombie && x.IsValidTarget(Q2.Range)))
                    {
                ...
                if (E2.IsReady() && useE2)
                {
                    foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsEnemy && !x.IsZombie && x.IsValidTarget(E2.Range)))
                    {
            ...
            if (unburrowed)
            {
                if (Q.IsReady() && useQ)
                {
                    foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsEnemy && !x.IsZombie))
                    {
                ...
                if (E.IsReady() && useE)
                {
                    foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsEnemy && !x.IsZombie && x.IsValidTarget(E.Range)))
                    {
                        E.Cast(enemy);
                    }
                }
