...
namespace LCS_Lucian
{
    class Helper
    {
        ...
        public static void LucianAntiGapcloser(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs spell)
        {
            ...
            {
                foreach (var gapclose in AntiGapcloseSpell.GapcloseableSpells.Where(x => spell.SData.Name == ((Obj_AI_Hero)sender).GetSpell(x.Slot).Name).OrderByDescending(c => LSlider("gapclose.slider." + spell.SData.Name)))
                {
                    if (LEnabled("gapclose." + ((Obj_AI_Hero)sender).ChampionName))
                    {
