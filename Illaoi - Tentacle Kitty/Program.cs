    ...
    class Program
    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (menuItem5.Active)
            {
                ...
                {
                    Render.Circle.DrawCircle(new Vector3(passive.Position.X, passive.Position.Y, passive.Position.Z), 820, menuItem5.Color, 2);
                    if (enemy != null)
                    {
                        var xx = Drawing.WorldToScreen(passive.Position.Extend(enemy.Position, 820));
        ...
        private static void Combo()
        {
            ...
            if (W.IsReady() && Config.Item("w.combo").GetValue<bool>())
            {
                ...
                if (tentacle != null)
                {
                    foreach (var enemy in HeroManager.Enemies.Where(x=> x.IsValidTarget(820)))
                    {
                        W.Cast();
                    }
                }
