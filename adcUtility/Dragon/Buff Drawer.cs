...
namespace adcUtility.Dragon
{
    public static class Buff_Drawer
    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            {
                foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsEnemy && x.HasBuff("s5test_dragonslayerbuff")))
                {
            ...
            {
                foreach (var ally in ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsMe && x.HasBuff("s5test_dragonslayerbuff")))
                {
