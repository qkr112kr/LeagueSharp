...
using HikiCarry_Lee_Sin;
using LeagueSharp;
using LeagueSharp.Common;
using HybridCommon;
using Geometry = HybridCommon.Geometry;
using SharpDX;
...
namespace HybridCommon
{
    ...
    public class Evader
    {
        ...
        private bool CorrectLessEnemies(ref Vector2 evade_pos, Vector2 direction)
        {
            ...
            {
                    evade_pos = ObjectManager.Player.Position.To2D() - direction * EvadeSpell.Range;
                    ObjectManager.Player.Spellbook.CastSpell(Items.GetWardSlot().SpellSlot, evade_pos.IsNormalized);
                    return true;
            }
