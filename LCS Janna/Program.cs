...
namespace LCS_Janna
{
    class Program
    {
        ...
        public static Obj_AI_Hero Udyr = ObjectManager.Player;
        ...
        private static void OnGameLoad(EventArgs args)
        {
            ...
            Config = new Menu("LCS Series: Janna", "LCS Series: Janna", true);
            {
        ...
        private static void OnProcess(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (E.IsReady())
            {
                ...
                if (sender is Obj_AI_Hero && sender.IsEnemy && args.Target.IsAlly && args.Target.Type == GameObjectType.obj_AI_Turret
                    && args.SData.IsAutoAttack() && ObjectManager.Player.ManaPercent >= Config.Item("min.mana.for.e").GetValue<Slider>().Value
                    && ((Obj_AI_Hero)args.Target).Distance(ObjectManager.Player.Position) < E.Range
                    && ((Obj_AI_Hero)args.Target).HealthPercent < Config.Item("turret.hp.percent").GetValue<Slider>().Value)
                {
                    E.Cast((Obj_AI_Hero)args.Target);
                }
            ...
            if (R.IsReady())
            {
                ...
                && sender.IsEnemy && sender.GetSpellDamage(((Obj_AI_Hero)args.Target), args.SData.Name) > ((Obj_AI_Hero)args.Target).Health
                && sender.GetSpellDamage(((Obj_AI_Hero)args.Target), args.SData.Name) * 100 / ((Obj_AI_Hero)args.Target).Health < Config.Item("spell.damage.percent").GetValue<Slider>().Value)
                {
                    R.Cast((Obj_AI_Hero)args.Target);
                }
            }
