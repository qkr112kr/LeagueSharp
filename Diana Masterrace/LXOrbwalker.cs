    ...
    public class LXOrbwalker
    {
        ...
        public static void Orbwalk(Vector3 goalPosition, Obj_AI_Base target)
        {
            ...
            if (!CanMove() || !IsAllowedToMove())
                return;
            if (MyHero.IsMelee() && target != null && target.Distance(MyHero,false) < GetAutoAttackRange(MyHero, target)
                ...
                foreach (
                    var minion in
                        ...
                        let t = (int)(MyHero.AttackCastDelay * 1000) - 100 + Game.Ping / 2 +
                                1000 * (int)MyHero.Distance(minion,false) / (int)MyProjectileSpeed()
